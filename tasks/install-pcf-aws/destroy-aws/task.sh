...
export VPC_ID=$(
  $root/terraform-bin/terraform state show -state "${root}/terraform-state/terraform.tfstate" aws_vpc.PcfVpc | grep ^id | awk '{print $3}'
)
...
set +e
$root/terraform-bin/terraform destroy \
  -force \
